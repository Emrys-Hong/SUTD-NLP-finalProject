...
def viterbi(x_instance, x_instance_pos, y_vocab, feature_dict):
    ...
    for i, y in enumerate(y_vocab):
        ...
        scores[0, i] = t_score + e_score + e_score_pos
    ...
    for i in range(1, n):
        for y_i, y in enumerate(y_vocab):
            for y_prev_i, y_prev in enumerate(y_vocab):
                ...
                score = t_score + e_score + e_score_pos + scores[i-1, y_prev_i]
    ...
    for i in range(1, n):
        for y_i, y in enumerate(y_vocab):
            ...
            for y_prev_i, y_prev in enumerate(y_vocab):
                ...
                temp.append(e_score + e_score_pos + t_score + scores[i-1, y_prev_i])
    ...
    for i in range(n-1, 0, -1):
        for y_i, y in enumerate(y_vocab):
            ...
            for y_next_i, y_next in enumerate(y_vocab):
                ...
                temp.append(e_score + e_score_pos + t_score + scores[i, y_next_i])
    ...
    for y_i, y in enumerate(y_vocab):
        for y_next_i, y_next in enumerate(y_vocab):
            ...
            for i in range(n-1):
                e_score = feature_dict.get(f"emission:{y}+{x_instance[i]}", ninf)
                e_score_pos = feature_dict.get(f"emission:{y}+{x_instance_pos[i]}", ninf)
                total += np.exp(f_scores[i, y_i] + b_scores[i+1, y_next_i] + t_score + e_score + e_score_pos - alpha)
...
if __name__ == "__main__":
    ...
    feature_dict = {**t_dict, **e_dict, **e_dict_pos}
    ...
    weight_name = save_dir/'full-part5-1.json'
    ...
    eval_filename = full_dir/'dev.p5.CRF.f3.out'
