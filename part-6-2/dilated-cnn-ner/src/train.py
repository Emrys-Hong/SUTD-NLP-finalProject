        ...
        with open(FLAGS.embeddings, 'r') as f:
            for line in f.readlines():
                ...
                if word in vocab_str_id_map:
                    ...
                    try:
                        embeddings[vocab_str_id_map[word]-1] = [float(i) for i in embedding]
                    except:
                        print(len(embedding), word)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_integer('embed_dim', 300, 'word embedding dimension')
    ...
    tf.app.flags.DEFINE_string('print_preds', 'pred.txt', 'print out predictions (for conll eval script) to given file (or do not if empty)')
