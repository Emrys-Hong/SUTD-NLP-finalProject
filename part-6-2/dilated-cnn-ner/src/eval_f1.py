...
#                         preds_nopad = map(lambda t: labels_id_str_map[t], preds[start:seq_len + start])
                        preds_nopad = [labels_id_str_map[t] for t in preds[start:seq_len + start]]
#                         labels_nopad = map(lambda t: labels_id_str_map[t], labels[start:seq_len + start])
                        labels_nopad = [labels_id_str_map[t] for t in labels[start:seq_len + start]]
#                         tokens_nopad = map(lambda t: vocab_id_str_map[t], tokens[start:seq_len + start])
                        tokens_nopad = [vocab_id_str_map[t] for t in tokens[start:seq_len + start]]
                        ...
                        for pred_conv, label_conv, pred, label, token in zip(preds_converted, labels_converted, preds_nopad,
                            ...
                            conll_preds_file.write("%s %s %s %s %s\n" % (token, label, pred, label_conv, pred_conv))
#                             print("%s %s %s %s %s " % (token, label, pred, label_conv, pred_conv))
#                         print("")
                        conll_preds_file.write("\n")
                        sentence_count += 1
