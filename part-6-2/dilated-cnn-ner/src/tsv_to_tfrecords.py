...
tf.app.flags.DEFINE_string('dataset', 'partial', 'which dataset')
...
# DOC_MARKER = DOC_MARKER_CONLL if FLAGS.dataset == "projectdata" else DOC_MARKER_ONTONOTES
DOC_MARKER = ''
...
def make_example(writer, lines, label_map, token_map, shape_map, char_map, update_vocab, update_chars):
    ...
    for i, line in enumerate(lines):
        line = line.strip()
        if line:
            token_str, label_str, current_tag = get_str_label_from_line_conll(line) if FLAGS.dataset == 'partial' else get_str_label_from_line_ontonotes(line, current_tag)
    ...
    intmapped_labels[pad_width:pad_width+len(labels)] = [label_map[s] for s in labels]#map(lambda s: label_map[s], labels)
...
def tsv_to_examples():
    ...
    # TODO refactor this!!!
    print("dataset name: %s"%FLAGS.dataset)
    if FLAGS.dataset == "partial":
